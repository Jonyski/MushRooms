...
function newNuclearCat(spawnPos, room)
	...
	local hitbox = hitbox(Rectangle.new(40, 70), spawnPos, ENEMY)
	local physics = physicsSettings(1, 65, 4)
	local enemy = Enemy.new(NUCLEAR_CAT.name, 30, spawnPos, physics, movementFunc, attack, hitbox, room)
...
function newSpiderDuck(spawnPos, room)
	...
	local hitbox = hitbox(Circle.new(25), spawnPos, ENEMY)
	local physics = physicsSettings(0.8, 50, 5)
	local enemy = Enemy.new(SPIDER_DUCK.name, 20, spawnPos, physics, movementFunc, attackFunc, hitbox, room)
