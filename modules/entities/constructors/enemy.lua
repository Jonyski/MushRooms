...
function newNuclearCat(spawnPos, room)
	local movementFunc = avoidTargetMovement(300, range(150, 180), Easing.outQuad)
	local attack = newPebbleShotAttack(false, 2.0, 12, sineMovement(math.rad(30)))
...
function newSpiderDuck(spawnPos, room)
	local movementFunc = dashToTargetMovement(Easing.outQuad)
	local attackFunc = newPebbleShotAttack(false, 1.0, 10, zigZagMovement())
